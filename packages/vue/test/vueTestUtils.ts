...
// copy form @vue/runtime-test
import { markRaw } from '@vue/reactivity'; // eslint-disable-line import/no-extraneous-dependencies
import { isOn } from '@vue/shared'; // eslint-disable-line import/no-extraneous-dependencies
...
function setText(node: TestText, text: string) {
  ...
  node.text = text; // eslint-disable-line no-param-reassign
}
...
function createAppWithConfig(comp: Component) {
  const instance = createApp(comp);
  instance.config.errorHandler = (err) => { throw err; };
  instance.config.warnHandler = () => { };
  return instance;
}
export { createAppWithConfig as createApp };
